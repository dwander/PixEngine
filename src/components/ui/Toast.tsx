...
export function Toast({ toast, onClose }: ToastProps) {
  ...
  return (
    <div
      className={`flex items-center gap-3 px-4 py-3 rounded-lg border ${getBgColor()} shadow-lg backdrop-blur-sm animate-in slide-in-from-top duration-300`}
    >
      {renderIcon()}
...
export function ToastContainer({ toasts, onClose }: ToastContainerProps) {
  return (
    <div className="fixed top-12 right-4 z-[10000] flex flex-col gap-2 w-96 max-w-[calc(100vw-2rem)]">
      {toasts.map((toast) => (
